...
This repo manages Claude Code settings across User and Project scopes, with provenance tracking for items adopted from external repos. Most changes happen through Claude Code or agent sessions. The conventions below codify what's converged on so far. Read these before making changes.
...
- `bin/adopt`, `bin/sources-index`, `bin/check-updates`. Provenance tooling. JSON sidecars plus an auto-generated `SOURCES.md`.
...
`/plugin install` updates `user/shared/settings.json`. Include that file in the same commit. Those entries are load-bearing beyond activation: `install.sh` and `uninstall.sh` resolve each plugin's marketplace from `enabledPlugins` and each third-party marketplace's repo from `extraKnownMarketplaces` to print exact commands. A plugin tracked under `user/shared/plugins/` but missing from every `enabledPlugins` (user scope, or a project template's) prints as a warning instead of a usable command.
...
`install.sh` and `uninstall.sh` print `/plugin install` reminders listing tracked plugins. Canonical examples: `user/shared/plugins/hookify/`, `user/shared/plugins/claude-md-management/`, `user/shared/plugins/commit-commands/`, `user/shared/plugins/skill-creator/`.
...
1. `mkdir -p user/shared/mcp/<name> && $EDITOR user/shared/mcp/<name>/README.md`. The README must include the exact `claude mcp add ...` command with placeholder values for any secrets (e.g., `<your-api-key>`).
   - **Gotcha:** `--header` and `-e` are variadic. Place them **after** `<name>` and `<commandOrUrl>` (or cap with `--`), or they'll greedily consume the positionals and emit `missing required argument 'name'`. Header values use HTTP-style `"Key: value"`, not `"Key=value"`.
...
`install.sh` and `uninstall.sh` print `claude mcp add` and `claude mcp remove` reminders listing tracked servers. Canonical example: `user/shared/mcp/context7/`.

### Add a self-authored item

Just create the file or folder under `user/shared/{skills,commands,agents,hooks,rules,output-styles}/`. No sidecar. In an opt-in scheme, "no provenance metadata" means "original work".

### Check for upstream updates

Run `bin/check-updates` or use the `/check-updates` skill in a Claude Code session. The script compares pinned commits against upstream HEAD. In `--deep` mode it also reports upstream entries added or removed next to tracked paths, meaning new or deleted skills, with a warning when a removed entry is adopted here. The skill layers judgment on top: whether tracked paths actually changed, and what the structural changes mean.

### Check that adopted items still work

Run `bin/check-integrity` (add `--strict` to fail on warnings too). It is static, offline, and deterministic, with no LLM, no network, and no side effects, so it is safe to run on every change.
...
- **Provenance sidecars** — valid JSON, non-empty `provenance[]`, required fields present. `"source": "self"` entries are exempt from `commit`, `path`, and `license` (see `docs/PROVENANCE.md` § "Edge cases").
...
**Changes that never touch `bin/adopt`**, such as a hand-written skill, an edited `SKILL.md`, or a deleted skill that others still reference, are covered by the project-scope `Stop` hook in `.claude/settings.json`. It runs the same check when a session here ends and blocks once, feeding the errors back so they get fixed while the session is still open. It stays silent when clean, and downgrades to a plain message if `stop_hook_active` is already set, so it cannot loop.
...
2. Write a short title and a 1-3 sentence summary (what was decided and why). Add the optional sections Status, Considered Options, and Consequences only when they add genuine value.
3. Commit with the change it documents.

### Update an adoption to a newer upstream commit

Re-run `bin/adopt` against the same destination with the new SHA. The sidecar's `provenance[]` list gets a new entry appended. The older entry stays as history. `bin/adopt` compares the old and new pins itself and warns when upstream added or removed entries next to the path (new or deleted sibling skills), or when an `inspired-by` path no longer exists at the new commit. A hand-run re-pin therefore gets the same structural check as `check-updates --deep` (ADR 0039).
...
- **Provenance lives in sidecars only.** Never inline in `SKILL.md`, agent, command, or `CLAUDE.md` frontmatter. Those files load into Claude's context when invoked, so embedded metadata becomes context noise.
- **Two `plugins/` directories. Don't confuse them.** `~/.claude/plugins/` is Claude Code's own plugin CLI state (registry, marketplaces, cache). It is never symlinked from here. `user/shared/plugins/` in this repo is provenance-only. No source is vendored, and activation is via `/plugin install`.
- **MCP secrets stay machine-local.** `user/shared/mcp/<name>/README.md` may contain registration commands but must use placeholders (`<your-api-key>`) for any secret values. The real `~/.claude.json` or `~/.claude/mcp.json` is on the "deliberately does NOT manage" list.
